		...
		<?php if ( $layout == 'special' ): ?>
				...
				<?php endif; ?>
					<div class="category-image">
						    ...
						    <?php if( !empty($image) && isset($image[0]) ) { ?>
		                        <img src="<?php echo esc_url_raw( $image[0] ); ?>" title="<?php echo esc_attr($term->name); ?>" style="max-width: 100%" alt="<?php echo esc_attr(get_bloginfo('name', 'display')); ?>">
			...
			<?php if ( isset($all_categories_page) && !empty($all_categories_page) ): ?>
				<div class="text-center clearfix text-more">
					<a class="btn btn-inverse-light btn-default" href="<?php echo esc_url( get_permalink( get_page_by_path( $all_categories_page ) ) ); ?>">
						<?php esc_html_e('All Categories', 'anton'); ?>
					</a>
				</div>
			<?php endif; ?>
		<?php else: ?>
				...
				<?php $i = 0; foreach ($categories as $term): ?>
					...
					<div class="col-md-<?php echo esc_attr( round(12/$columns) ); ?>">
						<div class="category-image">
							    ...
							    ?>
							    <?php if( !empty($image) && isset($image[0]) ) { ?>
			                        <img class="hidden-sm" src="<?php echo esc_url_raw( $image[0] ); ?>" title="<?php echo esc_attr($term->name); ?>" style="max-width: 100%" alt="<?php echo esc_attr(get_bloginfo('name', 'display')); ?>">
			...
			<?php if ( isset($all_categories_page) && !empty($all_categories_page) ): ?>
				<div class="text-center clearfix text-more">
					<a href="<?php echo esc_url( get_permalink( get_page_by_path( $all_categories_page ) ) ); ?>">
						<?php esc_html_e('All Categories', 'anton'); ?>
					</a>
				</div>
			<?php endif; ?>
			...
			<?php if ( isset($show_pagination) && !empty($show_pagination) ): ?>
				...
				<?php anton_fnc_pagination_custom_nav( $per_page, $found_posts, $max_num_pages, $paged );?>
				</div>
			<?php endif; ?>
		<?php endif; ?>
		</div>
