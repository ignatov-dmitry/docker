			...
			<?php foreach ($list_query as $key => $li) { ?>
				<div class="tab-pane<?php echo ($__count==0)?' active':''; ?>" id="<?php echo esc_attr($key).'-'.$_id; ?>">
					<div class="grid-wrapper">
							<?php
								$loop = anton_fnc_woocommerce_query($key,$number);
								if( $number == -1 )
								...
								if($loop->have_posts()){
									...
									//wc_get_template( 'widget-products/grid.php' , array( 'loop'=>$loop,'columns_count'=>$columns,'class_column'=>$class_column,'posts_per_page'=>$per_page ) ); 
								}
							?>
					</div>
