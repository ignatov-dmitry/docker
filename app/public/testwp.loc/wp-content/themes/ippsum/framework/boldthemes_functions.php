...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'ippsum' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'ippsum' ) );
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php echo esc_html__( 'Pingback:', 'ippsum' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'ippsum' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'ippsum' ), get_comment_date(), get_comment_time() ) . '</p>';
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'ippsum' ), $rating ) ?>">
									<span style="width:<?php echo esc_html( ( $rating / 5 ) * 100 ); ?>%"><strong itemprop="ratingValue"><?php echo esc_html( $rating ); ?></strong> <?php echo esc_html__( 'out of 5', 'ippsum' ); ?></span>
								</div>
							<?php }
						?>
					</div>
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'ippsum' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'ippsum' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'ippsum' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
...
if ( ! function_exists( 'boldthemes_set_global_uri' ) ) {
	function boldthemes_set_global_uri() {
		...
		$data .= 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'ippsum' ) . '\';';
		$data .= 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'ippsum' ) . '\';';
		return $data;
	}
}
...
if ( ! function_exists( 'boldthemes_get_post_author' ) ) {
	function boldthemes_get_post_author( $author_url = false, $prefix = 'by', $show_avatar = false ) {
		if ( $prefix == 'by' ) $prefix = esc_html__( 'by', 'ippsum' );
		$post = get_post();
		...
		if ( ! is_404() && ! is_front_page() ) {
			...
			if ( ! $simple ) {
				...
				if ( ! is_singular() || is_page() ) {
					$output .= '<span><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'ippsum' ) . '</a></span>';
				}
			} else {
				if ( ! is_singular() || is_page() ) {
					$output .= '<a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'ippsum' ) . '</a>';
				}
			}
			...
			if ( is_home() ) {
				...
				//if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
					...
					$sku = $product->get_sku() ? $product->get_sku() : esc_html__( 'N/A', 'ippsum' );
					$subtitle = '<span class = "btProductSKU">' . esc_html__( 'SKU:', 'ippsum' ) . ' ' . $sku . '</span>'; 
				...
				if ( ! is_null( boldthemes_get_option( 'pf_slug' ) ) && boldthemes_get_option( 'pf_slug' ) != '' ) {
					...
					$output .= $item_prefix . esc_html__( 'Portfolio', 'ippsum' ) . $item_suffix;
				} else {
					$output .= $item_prefix . esc_html__( 'Portfolio', 'ippsum' ) . $item_suffix;
				}
				...
				$output .= $item_prefix . esc_html__( 'Category', 'ippsum' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Tag', 'ippsum' ) . $item_suffix;
				
				$subtitle = '';
		  
			} else if ( is_author() ) {
			
				$output .= $item_prefix . esc_html__( 'Author', 'ippsum' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Search', 'ippsum' ) . $item_suffix;
